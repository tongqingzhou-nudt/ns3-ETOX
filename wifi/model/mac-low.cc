                           ...
                           MacLowTransmissionListener *listener)
{
  ...
  //for TxRatio:
  if(!m_currentHdr.GetAddr1().IsGroup() && !m_currentHdr.GetAddr1().IsBroadcast())
  {
	  if(m_txRatio)
		  m_txRatio->AddSendTo(m_currentHdr.GetAddr1());
	  else
		  NS_LOG_DEBUG("m_txRatio is empty");
  }
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
  ...
  if (hdr.IsRts ())
           ...
           && m_txParams.MustWaitAck ())
    {
	  ...
	  //for TxRatio:
	  if(m_txRatio)
		  m_txRatio->AddAckFrom(m_currentHdr.GetAddr1());
	  else
		  NS_LOG_DEBUG("m_txRatio is empty");
           ...
           && m_blockAckTimeoutEvent.IsRunning ())
    {
      ...
      //for TxRatio:
	  if(m_txRatio)
		  m_txRatio->AddAckFrom(hdr.GetAddr2());
	  else
		  NS_LOG_DEBUG("m_txRatio is empty");
...
//for TxRatio:
void
MacLow::SetTxRatio(Ptr<TxRatio> txRatio)
{
	m_txRatio = txRatio;
}
